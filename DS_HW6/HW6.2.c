...
void huffman_encoding(char *str, bits_stream *bits_str)
{
    ...
    for(int i=0;i<sizeof(str)+1;i++){
        ...
        for(int j=l;j<l+m_bit_size[n];j++){
            bits_str->stream[j]=m_LUT[n][j-l];
            bits_str->length++;
        }
...
int huffman_decoding(bits_stream *bits_str, TreeNode *node, char *decoded_str)
{
	...
	for(int k=0;k<bits_str->length;k++){
        if(bits_str->stream[k]==0){
            tr=tr->l;
            //printf("left");

            if(tr->l == NULL && tr->r == NULL){

            decoded_str[index_char++]= tr->data;
			tr = node;
...
void main()
{
	...
	// Generate the huffman codeword from the huffman binary tree
	huffman_traversal(element_root.ptree,huff1,top);
