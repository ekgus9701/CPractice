...
double random() {
	return rand() / (double)RAND_MAX; //RAND_MAX´Â ·£´ý µ¹·ÈÀ» ¶§ ³ª¿Ã ¼ö ÀÖ´Â ÃÖ´ñ°ª
}

// Various state variables needed for simulation
int duration = 15; // Simulation time
...
{	
	...
	while (clock < duration) {
		...
		if (service_time_mat[0] > 0)	// the customer is receiving service
			service_time_mat[0]--;
		...
		if (service_time_mat[1] > 0)	// the customer is receiving service
			service_time_mat[1]--;
